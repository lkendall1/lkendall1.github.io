...
An example: to calculate the average number of cases (`positiveIncrease`) in January and February, we use:
...
- Calculate the median number of cases (`positiveIncrease`) on the weekend by `month`. 
```{r}
df %>% 
  filter(weekend==1) %>% 
  group_by(month) %>% 
  summarise(median_cases = median(positiveIncrease))
```
...
```{r}
df %>% 
  filter(date >= '2020-06-01' & date < '2020-9-01') %>% 
  group_by(weekend) %>% 
  summarise(average_hospitalizations = mean(hospitalizedIncrease))
```
...
```{r}
df %>% 
  filter(date >= '2021-01-01' & date <= '2021-12-31', state == 'RI')   %>% 
  group_by(weekday) %>% 
  summarise(median_cases = median(positiveIncrease))
```
